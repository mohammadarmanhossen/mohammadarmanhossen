...
SRC = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "arman-prepped.png")
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "arman-ascii.svg")
...
parts.append(f'<style>.ascii {{ filter: drop-shadow(0px 1px 2px rgba(0,0,0,0.9)); }}</style>')
parts.append('<defs>'
             f'<pattern id="scanlines" width="4" height="4" patternUnits="userSpaceOnUse">'
             f'<line x1="0" y1="0" x2="4" y2="0" stroke="#000" stroke-opacity="0.15" stroke-width="2"/>'
             f'</pattern>'
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">arman@github: ~$ ./arman-hossen.sh</text>')
...
for ry, line in enumerate(rows_txt):
    ...
    text = (f'<text class="ascii" xml:space="preserve" x="{PAD}" y="{y:.1f}" fill="{INK}" '
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'arman@github:~$ whoami <tspan fill="{INK}">Md Arman Hossen</tspan></text>')
